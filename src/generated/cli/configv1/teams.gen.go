...
func init() { types.MustRegisterObject(TeamTypeMeta, &Team{}) }
...
func CreateTeam(
	...
	res, err := client.CreateTeam(&config_v1.CreateTeamParams{
		Context: ctx,
...
func newTeamCreateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			team, err = types.MustDecodeSingleObject[*Team](file, permissiveParsing)
			if err != nil {
				return err
			}
			...
			fullTeam, err := CreateTeam(ctx, client, team, dryRunFlags.DryRun)
			if err != nil {
				return err
			}

			if dryRunFlags.DryRun {
				stderr.Println("Team is valid and can be created")
				return nil
			}
			stderr.Printf("Team with slug %q created successfully\n", fullTeam.Spec.Slug)

			if err := outputFlags.WriteObject(fullTeam, cmd.OutOrStdout()); err != nil {
				return err
			}
			return nil
		},
...
func GetTeam(
	...
	res, err := client.ReadTeam(&config_v1.ReadTeamParams{
		Context: ctx,
		Slug:    slug,
	})
	if err != nil {
...
func newTeamReadCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entity, err := GetTeam(ctx, client, args[0])
			if err != nil {
				return err
			}
...
func newTeamUpdateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		GroupID: groups.Commands.ID,
		Short:   "Updates an existing Team.",
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx, cancel := context.WithTimeout(cmd.Context(), clientFlags.Timeout())
			defer cancel()
			if err := outputFlags.Validate(); err != nil {
			...
			fullTeam, err := UpdateTeam(ctx, client, team, updateOpts)
			if err != nil {
				return err
			}

			if dryRunFlags.DryRun {
				stderr.Println("Team is valid and can be updated")
				return nil
			}
			stderr.Printf("Team with slug %q applied successfully\n", fullTeam.Spec.Slug)
	...
	cmd.Flags().BoolVar(&createIfMissing, "create-if-missing", false, "If set, creates the Team if it does not already exist. Defaults to false.")
...
func DeleteTeam(
	...
	_, err := client.DeleteTeam(&config_v1.DeleteTeamParams{
		Context: ctx,
		Slug:    slug,
	})
	if err != nil {
...
func newTeamDeleteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "delete <slug>",
		GroupID: groups.Commands.ID,
		Short:   "Deletes a single Team by slug",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx, cancel := context.WithTimeout(cmd.Context(), clientFlags.Timeout())
			defer cancel()
			...
			_ = res
			fmt.Fprintf(cmd.OutOrStdout(), "deleted Team with slug %q\n", args[0])
...
func (r *TeamListOpts) registerFlags(flags *flag.FlagSet) {
	var emptyNames []string
	flags.StringSliceVar(&r.Names, "names", emptyNames, "Filters results by name, where any Team with a matching name in the given list (and matches all other filters) will be returned.")
	var emptySlugs []string
	flags.StringSliceVar(&r.Slugs, "slugs", emptySlugs, "Filters results by slug, where any Team with a matching slug in the given list (and matches all other filters) will be returned.")
...
func ListTeams(
	...
	for {
		res, err := client.ListTeams(&config_v1.ListTeamsParams{
			Context:     ctx,
			PageToken:   &nextToken,
		...
		pageMaxSize = pagination.CalculatePageSize(pagination.Calculation{
			GotItems:    gotItems,
			MaxItems:    opts.Limit,
			MaxPageSize: len(res.Payload.Teams),
		})
	}
}

func newTeamListCmd() *cobra.Command {
	var listOptions TeamListOpts
	clientFlags := client.NewClientFlags()
	outputFlags := output.NewFlags()

	cmd := &cobra.Command{
		Use:     "list",
		Short:   "Lists all Teams and applies optional filters",
		GroupID: groups.Commands.ID,
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx, cancel := context.WithTimeout(cmd.Context(), clientFlags.Timeout())
			defer cancel()
			...
			streamer := output.NewWriteObjectStreamer[*Team](writer)
			nextToken, err := ListTeams(ctx, client, streamer, listOptions)
			if err != nil {
				return err
			}
			...
			if nextToken != "" {
				nextPage := pagination.Result{
					...
					Message:       "There are additional teams. To view more, use the next page token or run the full command.",
...
func newTeamScaffoldCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "scaffold",
		GroupID: groups.Commands.ID,
		Short:   "Scaffolds a complete object with placeholder values",
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Fprint(cmd.OutOrStdout(), TeamScaffoldYAML)
		},
	}
	return cmd
}
