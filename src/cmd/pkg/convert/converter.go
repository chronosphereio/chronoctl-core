...
const (
	...
	defaultEntityGroupName = "Default"
	defaultEntityGroupSlug = "default"

	slugAllowedSet = "[a-zA-Z0-9-_]"
	separator      = "-"
)
...
var (
	...
	validSlugChars    = regexp.MustCompile(slugAllowedSet + "+")
...
type ConvertedAlerts struct {
	...
	NotificationPolicies []*models.Configv1NotificationPolicy
	Receivers            []*models.Configv1Notifier
...
func (c *Opts) severity(userSeverity string) (string, bool) {
	chronoSeverity := userSeverity
	if c.SeverityMappings != nil {
		if mappedSeverity, ok := c.SeverityMappings[userSeverity]; ok {
			chronoSeverity = mappedSeverity
		}
	}

	if !validSeverity(chronoSeverity) && chronoSeverity != allSeverities {
...
func (c *Opts) matchersWithSeverity(matchers []*labels.Matcher) (string, []*labels.Matcher) {
	severityLabel := c.SeverityLabelName
	if severityLabel == "" {
		severityLabel = severityLabelName
	}

	newMatchers := make([]*labels.Matcher, 0, len(matchers))
	var severityMatch string
	for _, matcher := range matchers {
		if matcher.Name == severityLabel && matcher.Type == labels.MatchEqual {
...
func ConvertPrometheus(
	...
	var alertReceivers []*models.Configv1Notifier
	var notificationPolicies []*models.Configv1NotificationPolicy
	...
	if len(promResult.monitors) > 0 {
		...
		if numTemplates := len(alertmgrConfig.Templates); numTemplates > 0 {
			// nil out the list of template files since they are not supported. When using chronoctl we
			// inline the template files into the notifier definitions (see chronoctl ExpandAlertManagerTemplates)
		...
		notificationPolicies = []*models.Configv1NotificationPolicy{policy}
	}
...
func (g groupTeamMatcher) Match(group string) (string, bool) {
	for _, groupTeam := range g {
		if groupTeam.GroupMatcher.MatchString(group) {
			return groupTeam.TeamSlug, true
		}
	}
	return "", false
}
...
func inferSignal(
	...
	for groupBy := range commonGroupBys {
		...
		if groupByString == alertNameLabelName || groupByString == severityLabelName {
			continue
		}
...
// Identical to dispatcher.Route but with references to multiReceiverRoute.
...
) (*models.Configv1NotificationPolicy, []*models.Configv1Notifier, []string, error) {
	var warnings []string
	if opts.ExpandContinueTrueRoutes {
		warnings = append(warnings, expandRoute(path, route, make(map[string]struct{}))...)
	}
	receivers, receiversByName, err := expandReceivers(config, opts)
	if err != nil {
...
func gatherPolicyOverrides(
	...
	receiversByName map[string][]*models.Configv1Notifier,
) (
	[]*policyOverride,
	[]string,
	error,
...
func copyNotifierList(nl *models.RoutesNotifierList) *models.RoutesNotifierList {
	return &models.RoutesNotifierList{
		NotifierSlugs:      append([]string{}, nl.NotifierSlugs...),
...
func mergeOverrides(a, b *policyOverride) (*policyOverride, bool) {
	...
	if b.Notifiers.Warn != nil {
		if override.Notifiers.Warn == nil {
			override.Notifiers.Warn = &models.RoutesNotifierList{}
		} else if override.Notifiers.Warn.RepeatIntervalSecs != b.Notifiers.Warn.RepeatIntervalSecs {
			return nil, false
		} else if override.Notifiers.Warn.GroupBy != b.Notifiers.Warn.GroupBy {
			return nil, false
		}
		override.Notifiers.Warn.NotifierSlugs = append(override.Notifiers.Warn.NotifierSlugs, b.Notifiers.Warn.NotifierSlugs...)
		slices.Compact(override.Notifiers.Warn.NotifierSlugs)
	}
	if b.Notifiers.Critical != nil {
		if override.Notifiers.Critical == nil {
			override.Notifiers.Critical = &models.RoutesNotifierList{}
		} else if override.Notifiers.Critical.RepeatIntervalSecs != b.Notifiers.Critical.RepeatIntervalSecs {
			return nil, false
		} else if override.Notifiers.Critical.GroupBy != b.Notifiers.Critical.GroupBy {
			return nil, false
		}
		override.Notifiers.Critical.NotifierSlugs = append(override.Notifiers.Critical.NotifierSlugs, b.Notifiers.Critical.NotifierSlugs...)
		slices.Compact(override.Notifiers.Critical.NotifierSlugs)
	}
	if b.AllSeverities != nil {
		if override.AllSeverities == nil {
			override.AllSeverities = &models.RoutesNotifierList{}
		} else if override.AllSeverities.RepeatIntervalSecs != b.AllSeverities.RepeatIntervalSecs {
			return nil, false
		} else if override.AllSeverities.GroupBy != b.AllSeverities.GroupBy {
			return nil, false
		}
		override.AllSeverities.NotifierSlugs = append(override.AllSeverities.NotifierSlugs, b.AllSeverities.NotifierSlugs...)
		slices.Compact(override.AllSeverities.NotifierSlugs)
	}
...
func convertPrometheusRuleGroups(
	...
	collectionUniqueFunc := inMemoryUnique()
	monitorSlugUniqueFunc := inMemoryUnique()
	...
	for i, ruleGroup := range promRules.Groups {
		...
		// Find the team for this rule group
		teamSlug, ok := opts.groupTeamMatcher.Match(ruleGroup.Name)
		if !ok {
			return nil, ruleGroupPath.field("name").invalidReason("No team found for rule group name `%s`", ruleGroup.Name)
		}

		// Create the slug first to pass into the convertRules function
		collectionSlug := generateSlug(ruleGroup.Name, collectionUniqueFunc)

		// Convert the rules to recording rules and monitors
		groupRecRules, groupMonitors, err := convertRules(ruleGroup, ruleGroupPath, opts, monitorSlugUniqueFunc, collectionSlug)
		if err != nil {
...
func convertRules(
	...
	opts Opts,
	monitorSlugUniqueFunc func(slug string) bool,
	parentSlug string,
) (
	...
	for i, rule := range ruleGroup.Rules {
		...
		if rule.Record.Value != "" {
			...
			if opts.AssignMonitorSlugs { //nolint:typecheck
				monitorSlug = generateSlug(rule.Alert.Value, monitorSlugUniqueFunc)
			}
...
func convertAlertingRule(
	...
	var nonSeverityLabels map[string]string
	...
	if !validSeverity(severity) {
		return nil, rulePath.field("labels").invalidReason("invalid %s value `%s`", severityLabelName, severity)
	}

	expr, op, threshold, err := parseExpr(rulePath.field("expr"), rule.Expr.Value, opts.ExistsOpSupported)
	if err != nil {
		...
		Conditions: []*models.MonitorCondition{
			{
				Op:                 op,
				Value:              threshold,
				SustainSecs:        int32(time.Duration(rule.For).Seconds()),
				ResolveSustainSecs: int32(time.Duration(rule.KeepFiringFor).Seconds()),
			},
		},
	}
	...
	monitor := &models.Configv1Monitor{
		...
		IntervalSecs:     int32(time.Duration(ruleGroup.Interval).Seconds()),
	}
...
func setReceiverSlugs(
	...
	for _, receiver := range receivers {
		...
		if slug, ok := opts.ReceiverNamesToSlugs[receiver.Name]; ok {
			...
			receiver.Slug = generateSlugV2(
				receiver.Name,
				func(slug string) bool {
					if _, ok := receiverSlugs[slug]; ok {
						return false
					}
...
func expandReceivers(
	...
	map[string][]*models.Configv1Notifier,
	error,
) {

	var allReceivers []*models.Configv1Notifier
	recvByInputName := make(map[string][]*models.Configv1Notifier, len(input.Receivers))
	...
	err := setReceiverSlugs(allReceivers, opts)
	if err != nil {
...
// expandReceiver takes a receiver that may have multiple targets and returns a set of receivers
// where each returned receiver has at most one target.
func expandReceiver(receiver config.Receiver) []*models.Configv1Notifier {
	var receivers []*models.Configv1Notifier
	for _, w := range receiver.WebhookConfigs {
		receivers = append(receivers, &models.Configv1Notifier{
			Name: receiver.Name,
	...
	for _, e := range receiver.EmailConfigs {
		receivers = append(receivers, &models.Configv1Notifier{
			Name: receiver.Name,
	...
	for _, s := range receiver.SlackConfigs {
	...
	for _, p := range receiver.PagerdutyConfigs {
	...
	for _, o := range receiver.OpsGenieConfigs {
	...
	for _, v := range receiver.VictorOpsConfigs {
		cfg := &models.NotifierVictorOpsConfig{
	...
	if len(receivers) == 0 {
		receivers = append(receivers, &models.Configv1Notifier{Name: receiver.Name})
	}
...
func validSeverity(severity string) bool {
	for _, validSev := range severities {
		if severity == validSev {
			return true
		}
	}
...
func inMemoryUnique() func(string) bool {
...
func generateSlug(name string, uniqueCheck func(slug string) bool) string {
	return generate(name, uniqueCheck, 36, false)
}

func generateSlugV2(name string, uniqueCheck func(slug string) bool) string {
	return generate(name, uniqueCheck, 200, true)
}

func generate(
	name string,
	uniqueCheck func(slug string) bool,
	maxLength int,
	convertUnicodeToASCII bool,
) string {
